...
Release 0.8 adds one deliberately narrow proactive behavior: deterministic announcements for newly current hostile or explicitly unknown contacts already admitted by the same own-side target-knowledge policy. It is not broad release-0.9 state-change notification work. The first accepted contact reconciliation after a session starts is a silent baseline. Later new contacts announce type and six-digit grid once; a last-known contact returning to current announces `contact reacquired`. Repeated snapshots, stale transitions, friendly/civilian/neutral records and malformed records remain silent. Deduplication and baseline state reset on session change.
