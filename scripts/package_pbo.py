#!/usr/bin/env python3
"""Create and validate a deterministic, uncompressed Arma addon PBO.

The fallback intentionally implements only the documented addon subset needed
by this repository: a Vers properties entry, uncompressed file entries, a
terminating zero entry and contiguous data. The optional trailing checksum is
omitted. Arma Tools AddonBuilder remains the preferred local packer.
"""
...
def pack(source: Path, output: Path, prefix: str) -> dict[str, object]:
    ...
    header = bytearray()
    # The Vers FourCC is serialized from integer 0x56657273 in little-endian order.
