...
using AICopilot.App.Models;
using AICopilot.App.Services;
...
public partial class MainWindow : Window
{
    ...
    private async void MainWindow_Loaded(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            _log.Info("AI Copilot v0.1.0 started.");
        }
        catch (Exception exception)
        {
            _log.Error("Startup failed", exception);
            MessageBox.Show(this, exception.Message, "AI Copilot startup error",
    ...
    private void OnTelemetryReceived(string json)
    {
        Dispatcher.Invoke(() =>
        {
            ...
            try
            {
                ...
                ContactsText.Text = root.TryGetProperty("contacts", out JsonElement contacts)
                    ? contacts.GetArrayLength().ToString()
                    : "0";
            ...
            {
                _log.Warn($"Received invalid telemetry JSON: {exception.Message}");
            }
        });
    }
    ...
    private static string BuildProbeSummary(JsonElement root)
    {
        if (!root.TryGetProperty("environment", out JsonElement environment) ||
            !environment.TryGetProperty("probes", out JsonElement probes) ||
    ...
    {
        if (!parent.TryGetProperty(propertyName, out JsonElement vector) ||
        ...
        return values.Length >= 2
            ? string.Join(" / ", values.Select(value => value.ToString("N1")))
            : "—";
    }
